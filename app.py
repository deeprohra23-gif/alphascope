    ...
    # ── Financial Snapshot & Projections ──
        try:
            ...
            if _financials is not None and not _financials.empty and len(_financials.columns) >= 3:
                ...
                def fmt_pe(val, forward=False):
                    if pd.isna(val) or val <= 0:
                        return '—'
                    return f'{val:.1f}x{"(F)" if forward else ""}'
                ...
                for col in fin_cols:
                    if pd.isna(col):
                        continue
                    fy_label = f"FY{str(col.year)[-2:]}" if hasattr(col, 'year') else str(col)[:4]

                    revenue = cr(get_val(_financials, 'Total Revenue', col))
                    net_profit = cr(get_val(_financials, 'Net Income', col))
                    if pd.isna(net_profit):
                        net_profit = cr(get_val(_financials, 'Net Income Common Stockholders', col))
                    ebitda = cr(get_val(_financials, 'EBITDA', col))
                    ebit = cr(get_val(_financials, 'EBIT', col))
                    interest_exp = cr(get_val(_financials, 'Interest Expense', col))

                    # EPS
                    eps_val = get_val(_financials, 'Diluted EPS', col)
                    if pd.isna(eps_val):
                        eps_val = get_val(_financials, 'Basic EPS', col)
                    ...
                    net_debt_val = np.nan
                    if col in bal_cols:
                        total_equity = cr(get_val(_balance, 'Stockholders Equity', col))
                        if pd.isna(total_equity):
                            total_equity = cr(get_val(_balance, 'Total Equity Gross Minority Interest', col))
                        total_debt = cr(get_val(_balance, 'Total Debt', col))
                        net_debt_val = cr(get_val(_balance, 'Net Debt', col))

                    # Cash flow
                    fcf = np.nan
                    if col in cf_cols:
                        fcf = cr(get_val(_cashflow, 'Free Cash Flow', col))

                    # Ratios
                    ebitda_margin = (ebitda / revenue * 100) if not pd.isna(ebitda) and not pd.isna(revenue) and revenue > 0 else np.nan
                    net_margin = (net_profit / revenue * 100) if not pd.isna(net_profit) and not pd.isna(revenue) and revenue > 0 else np.nan
                    de_ratio = (total_debt / total_equity) if not pd.isna(total_debt) and not pd.isna(total_equity) and total_equity > 0 else np.nan
                    nd_ebitda = (net_debt_val / ebitda) if not pd.isna(net_debt_val) and not pd.isna(ebitda) and ebitda > 0 else np.nan
                    int_coverage = (ebit / abs(interest_exp)) if not pd.isna(ebit) and not pd.isna(interest_exp) and abs(interest_exp) > 0 else np.nan
                    fcf_profit_pct = (fcf / net_profit * 100) if not pd.isna(fcf) and not pd.isna(net_profit) and net_profit > 0 else np.nan
                    roe = (net_profit / total_equity * 100) if not pd.isna(net_profit) and not pd.isna(total_equity) and total_equity > 0 else np.nan

                    # Historical PE — get price near March 31
                    hist_pe = np.nan
                    if not pd.isna(eps_val) and eps_val > 0 and _hist_prices is not None:
                        try:
                            target_date = col
                            nearby = _hist_prices.loc[_hist_prices.index.tz_localize(None) >= (target_date - timedelta(days=30)):_hist_prices.index.tz_localize(None) <= (target_date + timedelta(days=30))]
                            if nearby.empty:
                                nearby = _hist_prices.loc[:target_date.strftime('%Y-%m-%d')]
                            if not nearby.empty:
                                yr_end_price = nearby['Close'].iloc[-1]
                                hist_pe = yr_end_price / eps_val
                        except Exception:
                            pass

                    years_data.append({
                        'label': fy_label, 'year': col.year if hasattr(col, 'year') else 0,
                        'revenue': revenue, 'net_profit': net_profit, 'eps': eps_val,
                        'ebitda': ebitda, 'ebitda_margin': ebitda_margin, 'net_margin': net_margin,
                        'roe': roe, 'de_ratio': de_ratio, 'nd_ebitda': nd_ebitda,
                        'int_coverage': int_coverage, 'fcf': fcf, 'fcf_profit': fcf_profit_pct,
                        'pe': hist_pe, 'projected': False,
                    })

                # Filter to only years with actual data (revenue not NaN)
                years_data = [y for y in years_data if not pd.isna(y['revenue'])]
                years_data = sorted(years_data, key=lambda x: x['year'])

                # Take last 4 years max
                if len(years_data) > 4:
                    years_data = years_data[-4:]

                if len(years_data) >= 2:
                    # Calculate CAGRs for projection
                    first = years_data[0]
                    last = years_data[-1]
                    n_years = len(years_data) - 1

                    rev_cagr = ((last['revenue'] / first['revenue']) ** (1 / n_years) - 1) * 100 if first['revenue'] > 0 and last['revenue'] > 0 else np.nan
                    profit_cagr = ((last['net_profit'] / first['net_profit']) ** (1 / n_years) - 1) * 100 if first['net_profit'] > 0 and last['net_profit'] > 0 else np.nan
                    eps_cagr = ((last['eps'] / first['eps']) ** (1 / n_years) - 1) * 100 if not pd.isna(first['eps']) and not pd.isna(last['eps']) and first['eps'] > 0 and last['eps'] > 0 else np.nan

                    # Projection growth rate (use profit CAGR with 10% haircut, cap at 30%)
                    proj_rate = np.nan
                    if not pd.isna(profit_cagr) and profit_cagr > 0:
                        proj_rate = min(profit_cagr * 0.9, 30)  # 10% haircut, 30% cap
                    elif not pd.isna(rev_cagr) and rev_cagr > 0:
                        proj_rate = min(rev_cagr * 0.9, 30)

                    # Generate 2-year projections
                    if not pd.isna(proj_rate) and proj_rate > 0:
                        growth = 1 + proj_rate / 100
                        current_price_val = float(price) if not pd.isna(price) else np.nan

                        for yr_offset in range(1, 3):
                            proj_year = last['year'] + yr_offset
                            proj_rev = last['revenue'] * (growth ** yr_offset) if not pd.isna(last['revenue']) else np.nan
                            proj_profit = last['net_profit'] * (growth ** yr_offset) if not pd.isna(last['net_profit']) else np.nan
                            proj_eps = last['eps'] * (growth ** yr_offset) if not pd.isna(last['eps']) else np.nan
                            fwd_pe = (current_price_val / proj_eps) if not pd.isna(proj_eps) and proj_eps > 0 and not pd.isna(current_price_val) else np.nan

                            years_data.append({
                                'label': f"FY{str(proj_year)[-2:]}(E)",
                                'year': proj_year,
                                'revenue': proj_rev, 'net_profit': proj_profit, 'eps': proj_eps,
                                'ebitda': np.nan, 'ebitda_margin': np.nan, 'net_margin': np.nan,
                                'roe': np.nan, 'de_ratio': np.nan, 'nd_ebitda': np.nan,
                                'int_coverage': np.nan, 'fcf': np.nan, 'fcf_profit': np.nan,
                                'pe': fwd_pe, 'projected': True,
                            })

                    # ── Render Earnings Trajectory ──
                    header_cells = ''.join([f'<th style="padding:0.4rem 0.6rem;text-align:right;color:{"#ffaa33" if y["projected"] else "#e0e0e0"};font-size:0.72rem">{y["label"]}</th>' for y in years_data])

                    def make_row(label, key, formatter, suffix=''):
                        cells = ''
                        for y in years_data:
                            val = y.get(key, np.nan)
                            if y['projected'] and key in ('ebitda_margin', 'net_margin', 'roe', 'de_ratio', 'nd_ebitda', 'int_coverage', 'fcf', 'fcf_profit'):
                                cells += '<td style="padding:0.3rem 0.6rem;text-align:right;color:#333;font-size:0.72rem">—</td>'
                            else:
                                display = formatter(val) if not pd.isna(val) else '—'
                                color = '#ffaa33' if y['projected'] else '#ccc'
                                cells += f'<td style="padding:0.3rem 0.6rem;text-align:right;color:{color};font-size:0.72rem">{display}{suffix}</td>'
                        return f'<tr><td style="padding:0.3rem 0.6rem;color:#888;font-size:0.72rem;white-space:nowrap">{label}</td>{cells}</tr>'
                    ...
                    def make_pe_row():
                        ...
                        for y in years_data:
                            val = y.get('pe', np.nan)
                            color = '#ffaa33' if y['projected'] else '#ccc'
                            display = fmt_pe(val, y['projected'])
                            cells += f'<td style="padding:0.3rem 0.6rem;text-align:right;color:{color};font-size:0.72rem">{display}</td>'
                        return f'<tr><td style="padding:0.3rem 0.6rem;color:#888;font-size:0.72rem;white-space:nowrap">PE / Fwd PE</td>{cells}</tr>'

                    # CAGR summary line
                    cagr_parts = []
                    if not pd.isna(rev_cagr):
                        cagr_parts.append(f'Revenue CAGR {rev_cagr:.1f}%')
                    if not pd.isna(profit_cagr):
                        cagr_parts.append(f'Profit CAGR {profit_cagr:.1f}%')
                    if not pd.isna(proj_rate):
                        cagr_parts.append(f'Projection uses {proj_rate:.1f}% (conservative)')
                    cagr_line = ' · '.join(cagr_parts)

                    table_style = 'border-collapse:collapse;width:100%;font-family:IBM Plex Mono,monospace'
                    header_bg = 'background:#0f1117'

                    st.markdown(f"""
                    <div class="stock-card" style="margin-top:0.5rem">
                        <div class="stock-card-section">Earnings Trajectory</div>
                        <div style="overflow-x:auto">
                        <table style="{table_style}">
                            <tr style="{header_bg}"><th style="padding:0.4rem 0.6rem;text-align:left;color:#888;font-size:0.72rem"></th>{header_cells}</tr>
                            {make_row('Revenue (Cr)', 'revenue', fmt_cr)}
                            {make_row('Net Profit (Cr)', 'net_profit', fmt_cr)}
                            {make_row('EPS', 'eps', lambda v: f'{v:.1f}' if not pd.isna(v) else '—')}
                            {make_pe_row()}
                        </table>
                        </div>
                        <div style="font-size:0.6rem;color:#888;font-family:IBM Plex Mono,monospace;margin-top:0.4rem">{cagr_line}</div>
                    </div>
                    "", unsafe_allow_html=True)

                    # ── Key Ratios (Historical only) ──
                    hist_data = [y for y in years_data if not y['projected']]
                    hist_headers = ''.join([f'<th style="padding:0.4rem 0.6rem;text-align:right;color:#e0e0e0;font-size:0.72rem">{y["label"]}</th>' for y in hist_data])

                    def make_hist_row(label, key, formatter):
                        cells = ''
                        for y in hist_data:
                            val = y.get(key, np.nan)
                            display = formatter(val) if not pd.isna(val) else '—'
                            cells += f'<td style="padding:0.3rem 0.6rem;text-align:right;color:#ccc;font-size:0.72rem">{display}</td>'
                        return f'<tr><td style="padding:0.3rem 0.6rem;color:#888;font-size:0.72rem;white-space:nowrap">{label}</td>{cells}</tr>'

                    st.markdown(f"""
                    <div class="stock-card" style="margin-top:0.5rem">
                        <div class="stock-card-section">Key Ratios</div>
                        <div style="overflow-x:auto">
                        <table style="{table_style}">
                            <tr style="{header_bg}"><th style="padding:0.4rem 0.6rem;text-align:left;color:#888;font-size:0.72rem"></th>{hist_headers}</tr>
                            {make_hist_row('EBITDA Margin', 'ebitda_margin', fmt_pct)}
                            {make_hist_row('Net Margin', 'net_margin', fmt_pct)}
                            {make_hist_row('ROE', 'roe', fmt_pct)}
                        </table>
                        </div>
                    </div>
                    "", unsafe_allow_html=True)

                    # ── Financial Health (Historical only) ──
                    st.markdown(f"""
                    <div class="stock-card" style="margin-top:0.5rem">
                        <div class="stock-card-section">Financial Health</div>
                        <div style="overflow-x:auto">
                        <table style="{table_style}">
                            <tr style="{header_bg}"><th style="padding:0.4rem 0.6rem;text-align:left;color:#888;font-size:0.72rem"></th>{hist_headers}</tr>
                            {make_hist_row('Debt/Equity', 'de_ratio', fmt_ratio)}
                            {make_hist_row('Net Debt/EBITDA', 'nd_ebitda', lambda v: f'{v:.2f}x')}
                            {make_hist_row('Int Coverage', 'int_coverage', lambda v: f'{v:.1f}x')}
                            {make_hist_row('FCF (Cr)', 'fcf', fmt_cr)}
                            {make_hist_row('FCF/Profit', 'fcf_profit', fmt_pct)}
                        </table>
                        </div>
                        <div style="font-size:0.55rem;color:#555;font-family:IBM Plex Mono,monospace;margin-top:0.4rem">Projections based on historical growth trends. Actual results will vary. Not investment advice.</div>
                    </div>
                    "", unsafe_allow_html=True)
